...
// Variant: slower SPI + 8-byte sub-programming to reduce long continuous PP streams.
// -------------------------------
...
// Intentionally slow for breadboards + resistor dividers + long wires.
static const unsigned long SPI_HZ = 62500UL;
...
bool clearArrayProtection() {
  ...
  if ((readStatus1() & 0x7C) != 0) {
    setLastOpError("UNPROTECT_STILL_PROTECTED");
    return false;
  }
...
void handleWriteCommand(char *args) {
  ...
  // Program this host page in small sub-chunks. The datasheet permits 1 byte up to a page size
  // per PP operation, and smaller chunks are often more tolerant of marginal wiring.
...
void handleReadHexCommand(char *args) {
  char *addrToken = strtok(args, " ");
  char *lenToken = strtok(NULL, " ");

  if (addrToken == NULL || lenToken == NULL) {
    printError(F("READHEX_ARGS"));
    return;
  }
  ...
  if (length == 0 || length > PAGE_SIZE_BYTES) {
    printError(F("READHEX_LEN"));
    return;
  }
  if ((address + length) > FLASH_SIZE_BYTES) {
    printError(F("READHEX_RANGE"));
    return;
  }
...
void processLine(char *line) {
  ...
  if (strncmp(line, "READHEX ", 8) == 0) {
    handleReadHexCommand(line + 8);
    return;
  }
